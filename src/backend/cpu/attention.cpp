...
// K^T = [batch, num_kv_heads, seq_len_kv, head_dim] -> [batch, num_kv_heads, head_dim, seq_len_kv]; K.permute({0, 1, 3, 2})
// Q*K^T = [batch, num_heads, seq_len_q, head_dim] * [batch, num_kv_heads, head_dim, seq_len_kv] → [batch, num_heads, seq_len_q, seq_len_kv]
...
// 快速注意力机制 (Flash Attention) 实现
void flash_attention(Tensor* out) {
    const Tensor* Q = out->src[0];  // [batch, num_heads, seq_len_q, head_dim]
    const Tensor* K = out->src[1];  // [batch, num_kv_heads, seq_len_kv, head_dim]
    const Tensor* V = out->src[2];  // [batch, num_kv_heads, seq_len_kv, head_dim]
}
